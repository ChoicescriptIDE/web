...
function printImage(source, alignment) {
    ...
    if (scope.cside.getPlatform() && scope.cside.getPlatform() === "web-dropbox") {
...
window.allScenes = scope.cside.allScenes, Scene.prototype.lineMsg = function() {
    ...
    if (e.message = msg, msg) {
        if (/file/i.test(msg) && /exist/i.test(msg)) return void thisProject.logIssue(e);
    ...
    if (source = "file://" + thisProject.getPath() + source, ".mp3" === source.substring(source.length - 4, source.length)) {
        var scene = findScene(stats.sceneName);
        if (scene) {
            var e = {};
            e.lineNumber = stats.scene.lineNum + 1, e.message = "Unable to play proprietary format.\nPlease export game to test .mp3 audio.", 

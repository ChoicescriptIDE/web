...
function IDEViewModel() {
  ...
  function CSIDEScene(sceneData) {
    ...
    self.isLocked = ko.computed(function() {
      ...
      if ((curScene = cside.getSelectedScene()) && (curScene.beingEdited() && curScene != self)) return true;
      return false;
    }, this);
    ...
    self.rename = function(data, event) {
      ...
      /*if (event.type == "keyup" && event.keyCode != 13) { return; }
			...
			if (event.type != "dblclick") {
                ...
                //dblckick
                //ensure we're not already editing the name of another scene before allowing:
    ...
    function saveScene(callback) {
      ...
      function finalizeSave(err) {
        if (err) {
          //noty({type:"error", layout: "bottomLeft", text: "Error: " + err.message, closeWith: ["click"]});
          console.log(err);
        } else {
          dirty(false);
          cmDoc.markClean()
  ...
  insertTextTags = function(tagStart, tagEnd, allowSpecialLines) {
  ...
  if (usingNode) {
    ...
    self.cloneExample = function(data) {
      fh.selectFolder(function(newPath) {
        if (newPath) {
          __copyProjectTo(data.path, newPath, function(err) {
            if (err) {
              notification("Error", err.message, {
                type: "error"
              });
            ...
            var project;
            if ((project = projectExists(newPath))) {
              project.reloadAllScenes();
            } else {
              project = new CSIDEProject({
                name: data.title,
                path: newPath
              });
            ...
            notification("Import Successful", data.title + " imported to " + newPath, {
              type: "success"
            });
          });
        }
        ...
        "apply": function(channel) {
          if (channel != "none" && usingNode) {
            ...
            updater.checkForUpdates({
              ...
              n.close();
              if (err) {
                notification("Connection Error", "Failed to obtain update data from server", {
                  type: "error"
                });
              } else if (update) {
                var buttons = [{
    ...
    if (!usingNode) {
      db.authenticate({
        ...
        if (error) {
          ...
          db.getAccountInfo(function(error, accountInfo) {
            if (error) {
              //fail silently - it's not important functionality
            } else {
    ...
    if (typeof projectName === "string" && projectName != "") {
      ...
      bootbox.prompt({
        ...
        callback: function(projectName) {
          ...
          validName(projectName, true, function(valid) {
            if (valid) {
              __createProject(projectName);
            } else {
  ...
  function __addProject(project) {
    ...
    bootbox.alert("Error: Unable to add non-CSIDEProject.");
  }

  function __createProject(projectName) {
    function createProjectFolder(cb) {
    ...
    createProjectFolder(function(err, projectPath) {
      ...
      notification("Project created", project.getPath(), {
        type: 'success',
        layout: 'bottomRight'
      });
    });
  ...
  function getSceneName(scenePath) {
    ...
    return sceneName.substring(0, sceneName.length - 4).toLowerCase(); //gets rid of .txt extension + forces lowercase
  }
  ...
  function __testProject(project, test) {
    ...
    var path = ("lib/choicescript/" + test + "test.html");
    ...
    if (loadedSceneJS)
      ...
      $.getScript("lib/choicescript/web/scene.js")
        ...
        .fail(function(jqxhr, settings, exception) {
          if (typeof cb === "function") cb(new Error("<h3>Compilation Error</h3>" + "Failed to load scene.js."));
        })
    }

...
if (usingNode) {
    if (gui.App.on("open", function(path) {
        ".txt" == path.substring(path.lastIndexOf("."), path.length) && cside.openScene(path);
    }), gui.App.argv) for (var path = "", i = 0; i < gui.App.argv.length; i++) path = gui.App.argv[i], 
    ".txt" == path.substring(path.lastIndexOf("."), path.length) && cside.openScene(path);
...
Mousetrap.bind([ "command+s", "ctrl+s" ], function(e) {
    ...
    for (var file, fileList = e.dataTransfer.files, i = 0; i < fileList.length; i++) file = e.dataTransfer.files[i], 
    ".txt" == file.path.substring(file.path.lastIndexOf("."), file.path.length) && cside.openScene(file.path);
};
CodeMirror.defineMode("choicescript", function(conf, parserConf) {
    ...
    var words = parserConf.identifiers|| new RegExp("^[A-Za-z][A-Za-z]*'?[A-Za-z]*");
    ...
    function tokenBase(stream, state) {
        ...
        // Handle scope changes (sol = start of line)
         if (stream.sol()) {
			...
			console.log("STREAM: " + stream.indentation());
            if (stream.eatSpace()) {
...
/*         function tokenString(stream, state) {
    ...
    function tokenLexer(stream, state) {
		...
		console.log("stateDedent: " + state.dedent);
        if (state.dedent > 0 && stream.eol() && state.scopes[0].type == 'py') {
            if (state.scopes.length > 1) state.scopes.shift();
            state.dedent -= 1;
        }
		//highlight bad spelling hook
		if (!cside.spellCheck(current)) {
			return "spell-error";
		}
		else {
			return style;

...
exports.makeUniversalApp = async (opts) => {
    ...
    const x64AsarMode = await asar_utils_1.detectAsarMode(opts.x64AppPath, opts.x64AsarPath);
    const arm64AsarMode = await asar_utils_1.detectAsarMode(opts.arm64AppPath, opts.arm64AsarPath);
    ...
    try {
        ...
        const uniqueToX64 = [];
        const uniqueToArm64 = [];
        const x64Files = await file_utils_1.getAllAppFiles(await fs.realpath(tmpApp), opts.filesToSkip);
        const arm64Files = await file_utils_1.getAllAppFiles(await fs.realpath(opts.arm64AppPath), opts.filesToSkip);
        ...
        if (x64AsarMode === asar_utils_1.AsarMode.HAS_ASAR) {
            ...
            const x64AsarSha = await sha_1.sha((_a = opts.x64AsarPath) !== null && _a !== void 0 ? _a : path.resolve(tmpApp, 'Contents', 'Resources', 'app.asar'));
            const arm64AsarSha = await sha_1.sha((_b = opts.arm64AsarPath) !== null && _b !== void 0 ? _b : path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar'));

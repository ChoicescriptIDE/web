...
const implicitActivationEvent = vscode_1.l10n.t("This activation event cannot be explicitly listed by your extension.");
const redundantImplicitActivationEvent = vscode_1.l10n.t("This activation event can be removed as VS Code generates these automatically from your package.json contribution declarations.");
                    ...
                    for (const activationEventNode of activationEventsNode.children) {
                        ...
                        for (const implicitActivationEventPrefix of reservedImplicitActivationEventPrefixes) {
                            if (isImplicitActivationSupported && activationEvent.startsWith(implicitActivationEventPrefix)) {
                                const start = document.positionAt(activationEventNode.offset);
                                const end = document.positionAt(activationEventNode.offset + activationEventNode.length);
                                diagnostics.push(new vscode_1.Diagnostic(new vscode_1.Range(start, end), implicitActivationEvent, vscode_1.DiagnosticSeverity.Error));
                            }
                        }
                        // Star activation
                        if (activationEvent === '*') {
